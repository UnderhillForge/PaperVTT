...
func set_tool(tool_name: String, brush_size: float, brush_strength: float, flatten_height: float = 0.0) -> void:
...
func begin_stroke(world_position: Vector3) -> void:
	if not is_available():
		return
	_terrain.call("apply_brush", _tool_name, world_position, _brush_size, _brush_strength, _flatten_height)
