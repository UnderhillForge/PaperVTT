...
const TOD_MATH_SCRIPT: Script = preload("res://addons/sky_3d/src/TOD_Math.gd")
const SCATTER_LIB_SCRIPT: Script = preload("res://addons/sky_3d/src/ScatterLib.gd")
const MOON_TEXTURE_PATH: String = "res://addons/sky_3d/assets/thirdparty/textures/moon/MoonMap.png"
const STARMAP_TEXTURE_PATH: String = "res://addons/sky_3d/assets/thirdparty/textures/milkyway/Milkyway.jpg"
const STARFIELD_TEXTURE_PATH: String = "res://addons/sky_3d/assets/thirdparty/textures/milkyway/StarField.jpg"
const STARFIELD_NOISE_PATH: String = "res://addons/sky_3d/assets/textures/noise.jpg"
const CIRRUS_TEXTURE_PATH: String = "res://addons/sky_3d/assets/resources/SNoise.tres"
const CUMULUS_TEXTURE_PATH: String = "res://addons/sky_3d/assets/textures/noiseClouds.png"
...
func _build_scene() -> void:
	...
	var starfield_noise: Texture2D = _load_texture_or_null(STARFIELD_NOISE_PATH)
	if starfield_noise != null:
	...
	# Late-load texture defaults to avoid preload parse failures when import cache is stale.
	if moon_texture == null:
		moon_texture = _load_texture_or_null(MOON_TEXTURE_PATH)
	if cirrus_texture == null:
		cirrus_texture = _load_texture_or_null(CIRRUS_TEXTURE_PATH)
	if cumulus_texture == null:
		cumulus_texture = _load_texture_or_null(CUMULUS_TEXTURE_PATH)
	if starmap_texture == null:
		starmap_texture = _load_texture_or_null(STARMAP_TEXTURE_PATH)
	if star_field_texture == null:
		star_field_texture = _load_texture_or_null(STARFIELD_TEXTURE_PATH)
...
func _load_texture_or_null(path: String) -> Texture2D:
	# Prefer direct image decode for source image files so startup doesn't depend
	# on platform-specific .ctex cache variants already existing in .godot/imported.
	...
	push_warning("SkyDome failed to load texture: %s" % path)
	return null
...
func _update_sun_coords() -> void:
	...
	_sun_transform.origin = TOD_MATH_SCRIPT.spherical_to_cartesian(sun_altitude, sun_azimuth)
...
@export var moon_texture: Texture2D = null :
	set(value):
		moon_texture = value
...
func update_moon_coords() -> void:
	...
	_moon_transform.origin = TOD_MATH_SCRIPT.spherical_to_cartesian(moon_altitude, moon_azimuth)
...
@export var atm_wavelengths := Vector3(680.0, 550.0, 440.0) :
	set(value):
		...
		if is_scene_built:
			var wll: Vector3 = SCATTER_LIB_SCRIPT.compute_wavelenghts_lambda(atm_wavelengths)
			var wls: Vector3 = SCATTER_LIB_SCRIPT.compute_wavelenghts(wll)
			var betaRay: Vector3 = SCATTER_LIB_SCRIPT.compute_beta_ray(wls)
...
@export_range(0.0, 0.9999999, 0.0000001) var atm_sun_mie_anisotropy: float = 0.8 :
	set(value):
		...
		if is_scene_built:
			var partial: Vector3 = SCATTER_LIB_SCRIPT.get_partial_mie_phase(atm_sun_mie_anisotropy)
...
@export_range(0.0, 0.9999999, 0.0000001) var atm_moon_mie_anisotropy: float = 0.8 : 
	set(value):
		...
		if is_scene_built:
			var partial: Vector3 = SCATTER_LIB_SCRIPT.get_partial_mie_phase(atm_moon_mie_anisotropy)
...
func _update_beta_mie() -> void:
	if is_scene_built:
		var bm: Vector3 = SCATTER_LIB_SCRIPT.compute_beta_mie(atm_mie, atm_turbidity)
...
@export var cirrus_texture: Texture2D = null :
	set(value):
...
@export_range(0.0, 0.9999999, 0.0000001) var cumulus_mie_anisotropy: float = 0.206 :
	set(value):
		...
		if is_scene_built:
			var partial: Vector3 = SCATTER_LIB_SCRIPT.get_partial_mie_phase(cumulus_mie_anisotropy)
...
@export var cumulus_texture: Texture2D = null :
	set(value):
...
@export var starmap_texture: Texture2D = null :
	set(value):
...
@export var star_field_texture: Texture2D = null :
	set(value):
